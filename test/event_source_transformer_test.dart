  ...
  group('EventSourceTransformer', () {
    ...
    test('reuses last event id when field omitted', () async {
      ...
      final events = await Stream<List<int>>.fromIterable(
        [utf8.encode(buffer.toString())],
      ).transform(EventSourceTransformer()).toList();
    ...
    test('invokes retry indicator when retry field provided', () async {
      Duration? retry;
      final transformer = EventSourceTransformer(
    ...
    test('dispatches final event when stream closes without blank line', () async {
      final stream = Stream<List<int>>.fromIterable(
    ...
    test('wraps transformer errors in SseChannelException', () async {
      ...
      final transformed = controller.stream.transform(EventSourceTransformer());
